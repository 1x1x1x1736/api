...
namespace Seralyth.Mods
{
    ...
    {
        public static readonly Dictionary<(long, float), GameObject> auraPool =
            new Dictionary<(long, float), GameObject>();
        ...
        public static void VisualizeCube(Vector3 position, Quaternion rotation, Vector3 scale, Color color,
            long? indexId = null, float alpha = 0.25f)
        {
            long index = indexId ?? BitPackUtils.PackWorldPosForNetwork(position);
            var key = index;
        ...
        public static void VisualizeCylinder(Vector3 position, Quaternion rotation, Vector3 scale, Color color,
            long? indexId = null, float alpha = 0.25f)
        {
            long index = indexId ?? BitPackUtils.PackWorldPosForNetwork(position);
            var key = index;
        ...
        public static GameObject VisualizeCubeObject(Vector3 position, Quaternion rotation, Vector3 scale, Color color,
            float alpha = 0.25f)
        {
        ...
        public static void ConductDebug()
        {
            string text = "";
            text +=
                $"<color=blue><b>Seralyth</b></color> {PluginInfo.Version}  <color=grey>|</color>  Users Online:  {ServerData.onlineUsers}" +
                "\\n \\n";
            ...
            text += "<color=green>Color</color><color=grey>:</color> " + red + green + blue + " <color=grey>[</color>" +
                    redS + greenS + blueS + "<color=grey>]</color>\\n";

            string master = PhotonNetwork.InRoom && PhotonNetwork.IsMasterClient
                ? "<color=grey> [</color><color=red>Master</color><color=grey>]</color>"
                : "";
            text += "<color=green>Name</color><color=grey>:</color> " + PhotonNetwork.LocalPlayer?.NickName + master +
                    "\\n";

            text += "<color=green>ID</color><color=grey>:</color> " +
                    (Settings.hideId ? "Hidden" : PhotonNetwork.LocalPlayer?.UserId) + "\\n";
            text += "<color=green>Clip</color><color=grey>:</color> " + (GUIUtility.systemCopyBuffer?.Length > 35
                ? GUIUtility.systemCopyBuffer[..35]
                : GUIUtility.systemCopyBuffer) + "\\n";
            text += lastDeltaTime + " <color=green>FPS</color> <color=grey>|</color> " + PhotonNetwork.GetPing() +
                    " <color=green>Ping</color>\\n";

            string room = PhotonNetwork.InRoom
                ? NetworkSystem.Instance.SessionIsPrivate ? "Private" : "Public"
                : "Not in room";
            text += "<color=green>" +
                    NetworkSystem.Instance.regionNames[NetworkSystem.Instance.currentRegionIndex].ToUpper() +
                    "</color> " + PhotonNetwork.PlayerList.Length +
                    " <color=green>Players</color> <color=grey>|</color> " + room + "\\n \\n";

            string admin = "";
            if (Time.time > 5f)
            {
                if (ServerData.Administrators.TryGetValue(PhotonNetwork.LocalPlayer?.UserId ?? string.Empty,
                        out var administrator))
                    admin = " <color=grey>|</color> <color=red>Console " +
                            (ServerData.SuperAdministrators.Contains(administrator) ? "Super " : "") + "Admin</color>";
            }

            text += "<color=green>Theme</color> " + themeType + admin + "\n";
            text += "<color=green>Preferences Directory</color><color=grey>:</color> " +
                    $"{FileUtilities.GetGamePath()}/{PluginInfo.BaseDirectory}";

            GetObject("Environment Objects/LocalObjects_Prefab/TreeRoom/COCBodyText_TitleData")
                .GetComponent<TextMeshPro>().SafeSetText(text);
        }

        public static void ToggleSnow(bool enable)
        {
            GameObject snowObject =
                GetObject("Environment Objects/LocalObjects_Prefab/Forest/Environment/WeatherDayNight").transform
                    .Find("snow").gameObject;
        ...
        public static void WeatherChange(bool rain)
        {
            for (int i = 0; i < BetterDayNightManager.instance.weatherCycle.Length; i++)
                BetterDayNightManager.instance.weatherCycle[i] = rain
        ...
        public static void EnableFog() =>
            ZoneShaderSettings.activeInstance.SetGroundFogValue(new Color(0.9569f, 0.6941f, 0.502f, 0.1216f), 40f, 10f,
                40f);

        private static readonly Dictionary<TimeOfDayDependentAudio, float> ambientObjects =
            new Dictionary<TimeOfDayDependentAudio, float>();
        ...
        */
        ...
        public static void CoreESP()
        {
            ...
            List<GameEntity> cores = ManagerRegistry.GhostReactor.GameEntityManager.entities.Where(entity =>
                entity != null && entity.typeId == Overpowered.ObjectByName["GhostReactorCollectibleCore"]).ToList();
            if (cores.Count <= 0)
                return;
        ...
        public static void EnemyESP()
        {
            ...
            List<GameEntity> enemies = ManagerRegistry.GhostReactor.GameEntityManager.entities
                .Where(entity => entity != null && entity.gameObject.name.ToLower().Contains("enemy")).ToList();
            if (enemies.Count <= 0)
                return;
        ...
        public static void ResourceESP()
        {
            ...
            List<GameEntity> resources = ManagerRegistry.SuperInfection.GameEntityManager.entities
                .Where(entity => entity != null && entity.gameObject.name.Contains("Resource")).ToList();
        ...
        public static void RemoveBlindfold()
        {
            if (PhotonNetwork.InRoom && Time.time > removeBlindfoldDelay)
            {
                ...
                GameObject mainCamera =
                    GetObject("Player Objects/Player VR Controller/GorillaPlayer/TurnParent/Main Camera");
        ...
        public static TMP_SpriteAsset InfoSprites
        {
            get
            {
                if (_infoSpriteAsset == null)
                {
                    ...
                    AddSprite("Steam",
                        LoadTextureFromURL($"{PluginInfo.ServerResourcePath}/Images/Mods/Visuals/steam.png",
                            "Images/Mods/Visuals/steam.png"));
                    AddSprite("Standalone",
                        LoadTextureFromURL($"{PluginInfo.ServerResourcePath}/Images/Mods/Visuals/oculus.png",
                            "Images/Mods/Visuals/oculus.png"));
                    AddSprite("PC",
                        LoadTextureFromURL($"{PluginInfo.ServerResourcePath}/Images/Mods/Visuals/pc.png",
                            "Images/Mods/Visuals/pc.png"));
                    for (int i = 1; i <= 5; i++)
                        AddSprite($"Ping{i}",
                            LoadTextureFromURL($"{PluginInfo.ServerResourcePath}/Images/Mods/Visuals/ping{i}.png",
                                $"Images/Mods/Visuals/ping{i}.png"));
                    ...
                    Traverse.Create(_infoSpriteAsset).Field("m_Version")
        ...
        public static void WatchStep()
        {
            ...
            if (tmp)
            {
                bool defaultWatch = !infoWatchMenuName && !infoWatchTime && !infoWatchClip && !infoWatchFPS &&
                                    !infoWatchCode;
        ...
        public static void CustomSkyboxColor() =>
            GetObject("Environment Objects/LocalObjects_Prefab/Standard Sky").GetComponent<Renderer>().material =
                CustomBoardManager.BoardMaterial;
        ...
        public static void OnHandTapGamesenseRing(VRRig rig, Vector3 position)
        {
            ...
            if (tapTxt == null)
                tapTxt = LoadTextureFromURL($"{PluginInfo.ServerResourcePath}/Images/Mods/Visuals/footstep.png",
                    "footstep.png");
            if (warningTxt == null)
                warningTxt = LoadTextureFromURL($"{PluginInfo.ServerResourcePath}/Images/Mods/Visuals/warning.png",
                    "warning.png");
        ...
        public static void GamesenseRing()
        {
            for (int i = handTaps.Count - 1; i >= 0; i--)
            {
                ...
                float x = Vector3.Dot((worldPos - Camera.main.transform.position).normalized,
                ...
                obj.transform.position = Camera.main.transform.position + Camera.main.transform.forward * 0.5f
                                                                        + (Camera.main.transform.right * dir.x +
                                                                           Camera.main.transform.up * dir.y) * 0.2f;
                obj.transform.rotation =
                    Quaternion.LookRotation(obj.transform.position - Camera.main.transform.position,
        ...
        public static void ChangePerformanceModeVisualStep(bool positive = true)
        {
            ...
            Buttons.GetIndex("Change Performance Visuals Step").overlapText =
                "Change Performance Visuals Step <color=grey>[</color><color=green>" + PerformanceModeStep +
        ...
        public static void GetLabel(string codeName, bool leftHand, string text, Color color)
        {
            ...
            go.transform.position =
                (leftHand ? GorillaTagger.Instance.leftHandTransform : GorillaTagger.Instance.rightHandTransform)
                .position +
                Vector3.up * (GetLabelDistance(leftHand) * (scaleWithPlayer ? GTPlayer.Instance.scale : 1f));
        ...
        public static void VelocityLabel()
        {
            ...
            (
                ...
                GorillaTagger.Instance.rigidbody.linearVelocity.magnitude >= GTPlayer.Instance.maxJumpSpeed
                    ? Color.green
                    : Color.white
            );
        }
        ...
        public static void PingOverlay()
        {
            ...
            if (!PhotonNetwork.InRoom || PhotonNetwork.IsMasterClient || masterRig == null ||
                !playerPing.ContainsKey(masterRig))
            {
        ...
        public static void NearbyTaggerOverlay()
        {
            ...
            foreach (VRRig vrrig in VRRigCache.ActiveRigs)
            {
                ...
                {
                    float dist = Vector3.Distance(GorillaTagger.Instance.headCollider.transform.position,
        ...
        public static void InfoOverlayGun()
        {
            if (GetGunInput(false))
            {
                ...
                if (gunLocked && lockTarget != null)
                {
                    ...
                    NotificationManager.information["Turn"] =
                        $"{lockTarget.turnType.ToTitleCase()} {lockTarget.turnFactor}";
                }
        ...
        public static readonly Dictionary<SlingshotProjectile, LineRenderer> trajectoryPool =
            new Dictionary<SlingshotProjectile, LineRenderer>();
        ...
        public static void PaintbrawlTrajectories()
        {
            ...
            void LoopProjectileArray(LoopingArray<ProjectileTracker.ProjectileInfo> projectileArray)
            {
                ...
                for (int index = 0; index < projectileArray.Length; index++)
                {
                    ...
                    if ((VRRig.LocalRig.GetSlingshot() as Slingshot).dummyProjectile &&
                        (VRRig.LocalRig.GetSlingshot() as Slingshot).dummyProjectile
                        .GetComponent<SlingshotProjectile>() == projectileInstance) continue;
            ...
            foreach (LoopingArray<ProjectileTracker.ProjectileInfo> projectileArray in ProjectileTracker
                         .m_playerProjectiles.Values)
            ...
            foreach (VRRig rig in VRRigCache.ActiveRigs)
            {
                ...
                if (playerSlingshot != null)
                {
                    ...
                    {
                        SlingshotProjectile projectileInstance =
                            playerSlingshot.dummyProjectile.GetComponent<SlingshotProjectile>();
                        if (projectileInstance == null || !projectileInstance.gameObject.activeSelf) continue;
            ...
            DrawTrajectory(localPosition, localVelocity, localTrajectoryLine, NoInvisLayerMask(),
                Vector3.down * 10.79f);
        }
        ...
        public static void DrawTrajectory(Vector3 position, Vector3 velocity, LineRenderer lineRenderer,
            int? overrideLayerMask = null, Vector3? overrideGravity = null)
        {
            ...
            for (i = 0; i < stepCount; i++)
            {
                ...
                if (i >= 1 && Physics.Raycast(position, nextPosition - position, out RaycastHit hit,
                        (nextPosition - position).magnitude,
                        overrideLayerMask ?? GTPlayer.Instance.locomotionEnabledLayers))
                {
                    points[i] = hit.point;
                    i++;
        ...
        public static void VisualizeWindBarriers()
        {
            foreach (ForceVolume wind in GetAllType<ForceVolume>())
            {
                try
                {
                    VisualizeCube(wind.transform.position, wind.transform.rotation, wind.transform.lossyScale,
                        Color.blue);
                }
                catch
                {
        ...
        public static void VisualizeMapTriggers()
        {
            ...
            for (int i = 0; i < triggers.transform.childCount; i++)
            {
                try
                {
                    Transform child = triggers.transform.GetChild(i);
                    if (child.gameObject.activeSelf)
                        VisualizeCube(child.position, child.rotation, child.lossyScale,
                    ...
                    if (!vrrig.isLocal || selfNameTag)
                    {
                        ...
                        if (NameTagOptimize())
                        {
                            ...
                            if (cosmeticSet.items.Any(cosmetic =>
                                    !cosmetic.isNullItem && !vrrig.Cosmetics().Contains(cosmetic.itemName)))
                            {
                                if (specialMods == null)
                                    specialMods = "Cosmetx";
                                else
                ...
                try
                {
                    if ((!vrrig.isLocal || selfNameTag) && cosmetics == null)
                    {
                        ...
                        if (NameTagOptimize())
                        {
                            ...
                            foreach (var cosmetic in specialCosmetics.Where(cosmetic =>
                                         vrrig.Cosmetics().Contains(cosmetic.Key)))
                            {
                                if (cosmetics == null)
            ...
            foreach (VRRig vrrig in VRRigCache.ActiveRigs)
            {
                try
                {
                    if (!vrrig.isLocal || selfNameTag)
                    {
                        ...
                        {
                            double crashPower = Math.Abs(vrrig.velocityHistoryList[0].time * 1000 -
        ...
        private static readonly Dictionary<VRRig, GameObject> compactTagBackgrounds =
        ...
        public static string GetPrettyPing(VRRig vrrig)
        {
            ...
            return ping < 300 ? $"<color=green>{ping}</color>" :
                ping < 1000 ? $"<color=yellow>{ping}</color>" : $"<color=red>{ping}</color>";
        }
        ...
        public static string GetPrettyFPS(VRRig vrrig)
        {
            ...
            return fps < 30 ? $"<color=red>{fps}</color>" :
                fps < 60 ? $"<color=yellow>{fps}</color>" : $"<color=green>{fps}</color>";
        }
                ...
                try
                {
                    if (!vrrig.isLocal || selfNameTag)
                    {
                        if (!compactNameTags.ContainsKey(vrrig))
                        {
                            ...
                            infoTag.transform.localPosition =
                                new Vector3(0f, 0.5f,
                                    0f); // change the y to make the info tag farther or closer to the nametag 
                            ...
                            infoBg.transform.localPosition =
                                new Vector3(0f, 0.5f,
                                    0f); // change the y to make the info tag farther or closer to the nametag 
                            ...
                            rendererInfo.material.shader =
                                nameTagChams ? LoadAsset<Shader>("Chams") : Shader.Find("Sprites/Default");
        ...
        private static readonly Dictionary<VRRig, GameObject> minecraftTagBackgrounds =
            new Dictionary<VRRig, GameObject>();
        ...
        public static void FixRigColors()
        {
            foreach (var vrrig in VRRigCache.ActiveRigs.Where(vrrig =>
                         vrrig.mainSkin.material.name.Contains("gorilla_body") && vrrig.mainSkin.material.shader ==
        ...
        public static void DisableCosmetics()
        {
            try
            {
                ...
                {
                    if (Cosmetic.activeSelf &&
                        (Cosmetic.transform.parent == HeadCosmetics || Cosmetic.transform.parent == Head))
                    {
        ...
        public static void Xray()
        {
            if (rightTrigger > 0.5f)
            {
                ...
                {
                    foreach (Renderer renderer in GetAllType<Renderer>().Where(rend =>
                                 rend != null && rend.gameObject != null && !(rend is SkinnedMeshRenderer) &&
                                 rend.enabled && rend.gameObject.activeSelf))
                    {
            ...
            else
            {
                if (disabledRenderers.Count > 0)
                {
                    foreach (Renderer renderer in disabledRenderers.Where(rend =>
                                 rend != null && rend.gameObject != null))
        ...
        public static IEnumerator LerpRig(VRRig rig)
        {
            ...
            while (Time.time < startTime + length)
            {
                ...
                rig.rightHand.rigTarget.localRotation =
                    Quaternion.Lerp(rightHandStartRot, rig.rightHand.syncRotation, t);
        ...
        public static void CosmeticESP()
        {
            ...
            foreach (VRRig vrrig in VRRigCache.ActiveRigs)
            {
                ...
                if (!vrrig.isLocal && currentCosmetic != null)
                {
                    ...
                    {
                        texture = LoadTextureFromURL(
                            $"{PluginInfo.ServerResourcePath}/Images/Mods/Visuals/{currentCosmetic}.png",
                            $"Images/Mods/Visuals/{currentCosmetic}.png");
                    ...
                    indicator.transform.position = vrrig.headMesh.transform.position + vrrig.headMesh.transform.up *
                        (Classes.Menu.Console.GetIndicatorDistance(vrrig) * vrrig.scaleFactor);
                    indicator.transform.LookAt(GorillaTagger.Instance.headCollider.transform.position);
                }
                else if (currentCosmetic == null &&
                         cosmeticIndicators.TryGetValue(vrrig, out GameObject staleIndicator))
                {
        ...
        private static Texture2D GetPlatformTexture(VRRig rig)
        {
            ...
            {
                "Steam" => LoadTextureFromURL($"{PluginInfo.ServerResourcePath}/Images/Mods/Visuals/steam.png",
                    "Images/Mods/Visuals/steam.png"),
                "Standalone" => LoadTextureFromURL($"{PluginInfo.ServerResourcePath}/Images/Mods/Visuals/oculus.png",
                    "Images/Mods/Visuals/oculus.png"),
                "PC" => LoadTextureFromURL($"{PluginInfo.ServerResourcePath}/Images/Mods/Visuals/pc.png",
                    "Images/Mods/Visuals/pc.png"),
                _ => LoadTextureFromURL($"{PluginInfo.ServerResourcePath}/Images/Mods/Visuals/unknown.png",
                    "Images/Mods/Visuals/unknown.png"),
            };
        }
                    ...
                    if (size > 0f)
                    {
                        if (!voiceIndicators.TryGetValue(vrrig, out GameObject volIndicator))
                        {
                            ...
                            if (voiceMat == null)
                            {
                                ...
                                if (voiceTxt == null)
                                    voiceTxt = LoadTextureFromURL(
                                        $"{PluginInfo.ServerResourcePath}/Images/Mods/Visuals/speak.png",
                                        $"Images/Mods/Visuals/speak.png");
                        ...
                        volIndicator.transform.position = vrrig.headMesh.transform.position +
                                                          vrrig.headMesh.transform.up *
                                                          (Classes.Menu.Console.GetIndicatorDistance(vrrig) *
                                                           vrrig.scaleFactor);
        ...
        public static void NoLimbMode()
        {
            ...
            VRRig.LocalRig.mainSkin.material.color = new Color(VRRig.LocalRig.mainSkin.material.color.r,
                VRRig.LocalRig.mainSkin.material.color.g, VRRig.LocalRig.mainSkin.material.color.b, 0f);
            UpdateLimbColor();
        }
        ...
        public static void EndNoLimb()
        {
            ...
            VRRig.LocalRig.mainSkin.material.color = new Color(VRRig.LocalRig.mainSkin.material.color.r,
                VRRig.LocalRig.mainSkin.material.color.g, VRRig.LocalRig.mainSkin.material.color.b, 1f);
        }
        ...
        public static void CasualBoneESP()
        {
            ...
            foreach (var vrrig in VRRigCache.ActiveRigs.Where(vrrig => !vrrig.isLocal))
            {
                if (!boneESP.TryGetValue(vrrig, out List<LineRenderer> Lines))
                {
                    ...
                    for (int i = 0; i < 19; i++)
                    {
                        LineRenderer Line = vrrig.mainSkin.bones[bones[i * 2]].gameObject
        ...
        public static void HuntBoneESP()
        {
            ...
            foreach (var vrrig in VRRigCache.ActiveRigs.Where(vrrig => !vrrig.isLocal))
            {
                ...
                Color color = owner == target ? vrrig.GetColor() :
                    theirTarget == NetworkSystem.Instance.LocalPlayer ? Color.red : Color.clear;
        ...
        public static void CasualSkeletonESP()
        {
            foreach (var vrrig in VRRigCache.ActiveRigs.Where(vrrig => !vrrig.isLocal))
            {
                ...
                {
                    vrrig.skeleton.renderer.material.color = backgroundColor.GetCurrentColor();
                }

                if (Buttons.GetIndex("Transparent Theme").enabled)
                {
                    vrrig.skeleton.renderer.material.color = new Color(vrrig.skeleton.renderer.material.color.r,
                        vrrig.skeleton.renderer.material.color.g, vrrig.skeleton.renderer.material.color.b, 0.5f);
                }
            }
        }
            ...
            if (isInfectedPlayers)
            {
                if (!VRRig.LocalRig.IsTagged())
                {
                    foreach (VRRig vrrig in VRRigCache.ActiveRigs)
                    {
                        if (vrrig.IsTagged() && !vrrig.isLocal)
                        {
                            ...
                            if (Buttons.GetIndex("Transparent Theme").enabled)
                            {
                                vrrig.skeleton.renderer.material.color = new Color(
                                    vrrig.skeleton.renderer.material.color.r, vrrig.skeleton.renderer.material.color.g,
                                    vrrig.skeleton.renderer.material.color.b, 0.5f);
                            }
                        }
                        else
                        {
                ...
                else
                {
                    foreach (var vrrig in VRRigCache.ActiveRigs.Where(vrrig => !vrrig.IsTagged() && !vrrig.isLocal))
                    {
                        ...
                        {
                            vrrig.skeleton.renderer.material.color = new Color(vrrig.skeleton.renderer.material.color.r,
                                vrrig.skeleton.renderer.material.color.g, vrrig.skeleton.renderer.material.color.b,
                                0.5f);
                        }
                    }
                }
        ...
        private static readonly Dictionary<VRRig, SkinnedWireframeRenderer> wireframes =
            new Dictionary<VRRig, SkinnedWireframeRenderer>();
        ...
        public static void CasualWireframeESP()
        {
            ...
            foreach (var rig in VRRigCache.ActiveRigs.Where(rig => !rig.isLocal))
            {
                ...
                enabled &=
                    Vector3.Distance(rig.transform.position, GorillaTagger.Instance.headCollider.transform.position) <
                    35f &&
                    Vector3.Distance(rig.headMesh.transform.position,
                        GorillaTagger.Instance.headCollider.transform.position) < 35f &&
                    Vector3.Distance(rig.leftHandTransform.position,
                        GorillaTagger.Instance.headCollider.transform.position) < 35f &&
                    Vector3.Distance(rig.rightHandTransform.position,
                        GorillaTagger.Instance.headCollider.transform.position) < 35f;
        ...
        public static void HuntWireframeESP()
        {
            ...
            foreach (var rig in VRRigCache.ActiveRigs.Where(rig => !rig.isLocal))
            {
                ...
                Color color = owner == target ? rig.GetColor() :
                    theirTarget == NetworkSystem.Instance.LocalPlayer ? Color.red : Color.clear;
        ...
        {
            foreach (var vrrig in VRRigCache.ActiveRigs.Where(vrrig =>
                         !vrrig.isLocal && vrrig.colorInitialized && vrrig.initializedCosmetics &&
                         vrrig.mainSkin.material.shader.name != "Custom/UberChams"))
            {
                if (!uberChams)
                ...
                for (int i = 0; i < materials.Length; i++)
                    updateShader(materials[i],
                        i == 0 ? vrrig.materialsToChangeTo[vrrig.setMatIndex] : originalMaterials[bodyRenderer][i],
                        materials[i].color);
        ...
        public static void CasualChams()
        {
            foreach (var vrrig in VRRigCache.ActiveRigs.Where(vrrig => !vrrig.isLocal))
            {
                ...
                if (Buttons.GetIndex("Follow Menu Theme").enabled)
                {
                    vrrig.mainSkin.material.color = backgroundColor.GetCurrentColor();
                }

                if (Buttons.GetIndex("Transparent Theme").enabled)
                {
                    vrrig.mainSkin.material.color = new Color(vrrig.mainSkin.material.color.r,
                        vrrig.mainSkin.material.color.g, vrrig.mainSkin.material.color.b, 0.5f);
                }
            }
        }
        ...
        public static void CasualHollowBoxESP()
        {
            ...
            foreach (var vrrig in VRRigCache.ActiveRigs.Where(vrrig => !vrrig.isLocal))
            {
                if (!hollowBoxESP.TryGetValue(vrrig, out GameObject box))
                {
                    ...
                    outl.transform.localPosition = new Vector3(0f, -0.5f, 0f);
                    Object.Destroy(outl.GetComponent<BoxCollider>());
                    outl.transform.localScale =
                        new Vector3(thinTracers ? 1.025f : 1.1f, thinTracers ? 0.025f : 0.1f, 1f);
                    outl.transform.localRotation = Quaternion.identity;
                    outl.AddComponent<ClampColor>().targetRenderer = boxRenderer;
                    ...
                    Object.Destroy(outl.GetComponent<BoxCollider>());
                    outl.transform.localScale =
                        new Vector3(thinTracers ? 0.025f : 0.1f, thinTracers ? 1.025f : 1.1f, 1f);
                    outl.transform.localRotation = Quaternion.identity;
                    outl.AddComponent<ClampColor>().targetRenderer = boxRenderer;
        ...
        public static void NearestTracer()
        {
            ...
            foreach (var rig in VRRigCache.ActiveRigs.Where(rig =>
        ...
        public static void FarthestTracer()
        {
            ...
            float farthest = 0f;
            VRRig farthestRig = null;
            foreach (var rig in VRRigCache.ActiveRigs)
            {
                if (rig.isLocal) continue;
                float dist = Vector3.Distance(rig.transform.position, VRRig.LocalRig.transform.position);
                if (dist > farthest)
                {
                    farthest = dist;
                    farthestRig = rig;
            ...
            if (farthestRig != null)
            {
                Color lineColor = farthestRig.playerColor;
                ...
                line.endWidth = lineWidth;
                line.SetPosition(0, GorillaTagger.Instance.rightHandTransform.position);
                line.SetPosition(1, farthestRig.transform.position);
            }
        }

        // Nearest/Farthest ESP variants
        private static float GetDist(VRRig rig) =>
            Vector3.Distance(rig.transform.position, VRRig.LocalRig.transform.position);
        ...
        public static void NearestBoxESP()
        {
            ...
            bool tt = Buttons.GetIndex("Transparent Theme").enabled;
            List<VRRig> toRemove = new List<VRRig>();
            foreach (var b in boxESP.Where(box => !VRRigCache.ActiveRigs.Contains(box.Key)))
            {
                toRemove.Add(b.Key);
                Object.Destroy(b.Value);
            }

            foreach (VRRig rig in toRemove) boxESP.Remove(rig);
            VRRig target = FindNearest();
            toRemove.Clear();
            foreach (var kvp in boxESP)
            {
                if (kvp.Key != target)
                {
            ...
            if (!boxESP.TryGetValue(target, out GameObject box))
            {
                box = GameObject.CreatePrimitive(PrimitiveType.Cube);
                Object.Destroy(box.GetComponent<BoxCollider>());
                box.transform.localScale = new Vector3(0.5f, 0.5f, 0f);
                box.GetComponent<Renderer>().material.shader = Shader.Find("GUI/Text Shader");
                boxESP.Add(target, box);
            }

            Color color = target.playerColor;
            if (fmt) color = backgroundColor.GetCurrentColor();
            if (tt) color.a = 0.5f;
            if (hoc) box.layer = 19;
            box.GetComponent<Renderer>().material.color = color;
        ...
        public static void NearestHollowBoxESP()
        {
            ...
            bool thinTracers = Buttons.GetIndex("Thin Tracers").enabled;
            List<VRRig> toRemove = new List<VRRig>();
            foreach (var b in hollowBoxESP.Where(box => !VRRigCache.ActiveRigs.Contains(box.Key)))
            {
                toRemove.Add(b.Key);
                Object.Destroy(b.Value);
            }

            foreach (VRRig rig in toRemove) hollowBoxESP.Remove(rig);
            VRRig target = FindNearest();
            toRemove.Clear();
            foreach (var kvp in hollowBoxESP)
            {
                if (kvp.Key != target)
                {
                    toRemove.Add(kvp.Key);
            ...
            foreach (VRRig rig in toRemove) hollowBoxESP.Remove(rig);
            if (target == null) return;
            if (!hollowBoxESP.TryGetValue(target, out GameObject box))
            {
                box = GameObject.CreatePrimitive(PrimitiveType.Cube);
                box.transform.position = target.transform.position;
                Object.Destroy(box.GetComponent<BoxCollider>());
        ...
        public static void NearestBreadcrumbs()
        {
            ...
            foreach (VRRig rig in toRemove) breadcrumbs.Remove(rig);
            VRRig target = FindNearest();
            toRemove.Clear();
            foreach (var kvp in breadcrumbs)
            {
                if (kvp.Key != target)
                {
                    toRemove.Add(kvp.Key);
            ...
            bool thinTracers = Buttons.GetIndex("Thin Tracers").enabled;
            bool shortBreadcrumbs = Buttons.GetIndex("Short Breadcrumbs").enabled;
            if (!breadcrumbs.TryGetValue(target, out TrailRenderer trail))
            {
                trail = target.head.rigTarget.gameObject.GetOrAddComponent<TrailRenderer>();
                trail.minVertexDistance = 0.05f;
                if (smoothLines)
                {
                    trail.numCapVertices = 10;
                ...
                trail.material.shader = Shader.Find("GUI/Text Shader");
                trail.time = shortBreadcrumbs ? 1f : 10f;
                breadcrumbs.Add(target, trail);
            }

            trail.startWidth = thinTracers ? 0.0075f : 0.025f;
            trail.endWidth = thinTracers ? 0.0075f : 0.025f;
            if (hoc) trail.gameObject.layer = 19;
            Color color = target.GetColor();
            if (fmt) color = backgroundColor.GetCurrentColor();
            if (tt) color = new Color(color.r, color.g, color.b, 0.5f);
            trail.startColor = color;
            trail.endColor = color;
        }
        ...
        public static void NearestBoneESP()
        {
            ...
            {
                toRemove.Add(boness.Key);
                foreach (LineRenderer renderer in boness.Value) Object.Destroy(renderer);
            }

            foreach (VRRig rig in toRemove) boneESP.Remove(rig);
            VRRig target = FindNearest();
            toRemove.Clear();
            foreach (var kvp in boneESP)
            {
                ...
                {
                    toRemove.Add(kvp.Key);
                    foreach (LineRenderer renderer in kvp.Value) Object.Destroy(renderer);
                }
            }

            foreach (VRRig rig in toRemove) boneESP.Remove(rig);
            if (target == null) return;
            if (!boneESP.TryGetValue(target, out List<LineRenderer> Lines))
            {
                Lines = new List<LineRenderer>();
                LineRenderer LineHead = target.head.rigTarget.gameObject.GetOrAddComponent<LineRenderer>();
                if (smoothLines)
                {
                    LineHead.numCapVertices = 10;
                    LineHead.numCornerVertices = 5;
                ...
                for (int i = 0; i < 19; i++)
                {
                    LineRenderer Line = target.mainSkin.bones[bones[i * 2]].gameObject
                        .GetOrAddComponent<LineRenderer>();
                    if (smoothLines)
                    {
                        Line.numCapVertices = 10;
            ...
            if (fmt) color = backgroundColor.GetCurrentColor();
            if (tt) color.a = 0.5f;
            if (hoc) liner.gameObject.layer = 19;
            liner.startWidth = thinTracers ? 0.0075f : 0.025f;
            liner.endWidth = thinTracers ? 0.0075f : 0.025f;
            liner.startColor = color;
            liner.endColor = color;
            liner.SetPosition(0, target.head.rigTarget.transform.position + new Vector3(0f, 0.16f, 0f));
            liner.SetPosition(1, target.head.rigTarget.transform.position - new Vector3(0f, 0.4f, 0f));
            for (int i = 0; i < 19; i++)
            {
                liner = Lines[i + 1];
                if (hoc) liner.gameObject.layer = 19;
                ...
                liner.endColor = color;
                liner.material.shader = Shader.Find("GUI/Text Shader");
                liner.SetPosition(0, target.mainSkin.bones[bones[i * 2]].position);
                liner.SetPosition(1, target.mainSkin.bones[bones[i * 2 + 1]].position);
            }
        }
        ...
        public static void NearestSkeletonESP()
        {
            ...
            VRRig target = FindNearest();
            if (target == null) return;
            target.skeleton.renderer.enabled = true;
            target.skeleton.renderer.material.shader = Shader.Find("GUI/Text Shader");
            target.skeleton.renderer.material.color = target.playerColor;
            if (Buttons.GetIndex("Follow Menu Theme").enabled)
                target.skeleton.renderer.material.color = backgroundColor.GetCurrentColor();
            if (Buttons.GetIndex("Transparent Theme").enabled)
                target.skeleton.renderer.material.color = new Color(target.skeleton.renderer.material.color.r,
                    target.skeleton.renderer.material.color.g, target.skeleton.renderer.material.color.b, 0.5f);
        }
        ...
        public static void NearestWireframeESP()
        {
            ...
            foreach (VRRig rig in toRemove) wireframes.Remove(rig);
            VRRig target = FindNearest();
            toRemove.Clear();
            foreach (var kvp in wireframes)
            {
                if (kvp.Key != target)
                {
                    toRemove.Add(kvp.Key);
            ...
            bool hoc = Buttons.GetIndex("Hidden on Camera").enabled;
            bool tt = Buttons.GetIndex("Transparent Theme").enabled;
            if (!wireframes.TryGetValue(target, out SkinnedWireframeRenderer wireframe))
            {
                wireframe = target.mainSkin.AddComponent<SkinnedWireframeRenderer>();
                wireframes.Add(target, wireframe);
            }

            if (hoc) wireframe.wireframeObj.layer = 19;
            Color color = target.GetColor();
            if (fmt) color = backgroundColor.GetCurrentColor();
            if (tt) color = new Color(color.r, color.g, color.b, 0.5f);
            wireframe.meshRenderer.material.color = color;
            Vector3 toTarget = target.transform.position - GorillaTagger.Instance.headCollider.transform.position;
            float angle = Vector3.Angle(GorillaTagger.Instance.headCollider.transform.forward, toTarget);
            bool enabled = angle <= Camera.main.fieldOfView / 1.75f;
            enabled &= Vector3.Distance(target.transform.position,
                GorillaTagger.Instance.headCollider.transform.position) < 35f;
            enabled &= Vector3.Distance(target.headMesh.transform.position,
                GorillaTagger.Instance.headCollider.transform.position) < 35f;
            enabled &= Vector3.Distance(target.leftHandTransform.position,
                GorillaTagger.Instance.headCollider.transform.position) < 35f;
            enabled &= Vector3.Distance(target.rightHandTransform.position,
                GorillaTagger.Instance.headCollider.transform.position) < 35f;
            wireframe.enabled = enabled;
            wireframe.meshRenderer.enabled = enabled;
            if (!enabled)
            {
                FixRigMaterialESPColors(target);
                target.mainSkin.material.shader = Shader.Find("GUI/Text Shader");
                target.mainSkin.material.color = color;
            }
            else
            {
                target.mainSkin.material.shader = Shader.Find("GorillaTag/UberShader");
                if (target.mainSkin.material.name.Contains("gorilla_body"))
                    target.mainSkin.material.color = target.playerColor;
            }
        }
        ...
        public static void NearestChams()
        {
            ...
            target.mainSkin.material.color = target.playerColor;
            if (Buttons.GetIndex("Follow Menu Theme").enabled)
                target.mainSkin.material.color = backgroundColor.GetCurrentColor();
            if (Buttons.GetIndex("Transparent Theme").enabled)
                target.mainSkin.material.color = new Color(target.mainSkin.material.color.r,
                    target.mainSkin.material.color.g, target.mainSkin.material.color.b, 0.5f);
        }
        ...
        public static void NearestBeacons()
        {
            ...
            bool thinTracers = Buttons.GetIndex("Thin Tracers").enabled;
            Color menuColor = backgroundColor.GetCurrentColor();
            Color lineColor = target.playerColor;
            LineRenderer line = GetLineRender();
            if (followMenuTheme) lineColor = menuColor;
            ...
            line.startWidth = width;
            line.endWidth = width;
            line.SetPosition(0, target.transform.position + new Vector3(0f, 9999f, 0f));
            line.SetPosition(1, target.transform.position - new Vector3(0f, 9999f, 0f));
        }
        ...
        public static void NearestDistanceESP()
        {
            ...
            Color bgColor = target.playerColor;
            if (followMenuTheme) bgColor = menuColor;
            if (transparentTheme)
            {
                bgColor.a = 0.5f;
                tagColor.a = 0.5f;
            ...
            nameTagText.gameObject.transform.position = target.transform.position + new Vector3(0f, -0.2f, 0f);
            nameTagText.color = tagColor;
            _ = $"{Vector3.Distance(Camera.main.transform.position, target.transform.position):F1}m";
            foreach (Transform transform in nameTagText.gameObject.GetComponentsInChildren<Transform>())
            {
                if (transform.gameObject.name == "bg")
                {
                    transform.gameObject.GetComponent<Renderer>().material.color = bgColor;
                    transform.localScale = new Vector3(nameTagText.GetComponent<Renderer>().bounds.size.x + 0.2f, 0.2f,
                        0.01f);
                }
            }
        }
        ...
        public static void CasualDistanceESP()
        {
            ...
            foreach (VRRig playerRig in VRRigCache.ActiveRigs)
            {
                ...
                foreach (Transform transform in
                         nameTagText.gameObject.GetComponentsInChildren<Transform>()) //background color
                {
                    if (transform.gameObject.name == "bg")
                    {
                        transform.gameObject.GetComponent<Renderer>().material.color = backgroundColor;
                        transform.localScale = new Vector3(nameTagText.GetComponent<Renderer>().bounds.size.x + 0.2f,
                            0.2f, 0.01f);
                    }
                }
            }
        ...
        public static void HuntDistanceESP()
        {
            ...
            foreach (var playerRig in VRRigCache.ActiveRigs.Where(playerRig => !playerRig.isLocal))
            {
                if (GetPlayerFromVRRig(playerRig) == currentTarget) // Use ID for quick comparison
                {
                    ...
                    nameTagText.gameObject.transform.position =
                        playerRig.transform.position + new Vector3(0f, -0.2f, 0f);
                    nameTagText.color = tagColor;
                             ...
                             nameTagText.gameObject.GetComponentsInChildren<Transform>()) // Background color
                    {
                        if (transform.gameObject.name == "bg")
                        {
                            ...
                            transform.localScale =
                                new Vector3(nameTagText.GetComponent<Renderer>().bounds.size.x + 0.2f, 0.2f, 0.01f);
                        }
                    }
                }
                else if (sillyComputer.IsTargetOf(GetPlayerFromVRRig(playerRig), PhotonNetwork.LocalPlayer))
        ...
        private static TextMeshPro GetNameTag(bool hideOnCamera)
        {
            ...
            foreach (var TextMeshPro in nameTagPool.Where(TextMeshPro =>
                         finalTextMeshPro == null && !TextMeshPro.gameObject.activeInHierarchy))
            {
            ...
            finalTextMeshPro.gameObject.layer = hideOnCamera
                ? 19
                : // What does 19 actually do?
                nameTagHolder.layer;
        ...
        public static void
            ClearNameTagPool(
                bool destroy = false) // Set destroy when you disable a feature that needs a lot of nameTags
        {
            if (DoPerformanceCheck())
                return;
        ...
        public static LineRenderer GetLineRender()
        {
            ...
            foreach (var line in linePool.Where(line => finalRender == null)
                         .Where(line => !line.gameObject.activeInHierarchy))
            {
                line.gameObject.SetActive(true);
                finalRender = line;
            }
        ...
        public static void
            ClearLinePool(bool destroy = false) // Set destroy when you disable a feature that needs a lot of lines
        {
            if (DoPerformanceCheck())
                return;
        ...
        public static void ConsoleBeacon(string id, string version, string menuName)
        {
            ...
            NotificationManager.SendNotification(
                "<color=grey>[</color><color=purple>ADMIN</color><color=grey>]</color> " + sender.NickName +
                " is using " + menuName + " version " + version + ".", 3000);
        ...
        {
            bool gunInput = Main.GetGunInput(false);
            if (gunInput)
            {
                ValueTuple<RaycastHit, GameObject> valueTuple = Main.RenderGun(null);
                RaycastHit item = valueTuple.Item1;
                bool gunInput2 = Main.GetGunInput(true);
                if (gunInput2)
                {
                    VRRig componentInParent = item.collider.GetComponentInParent<VRRig>();
                    bool flag = componentInParent && !componentInParent.isOfflineVRRig;
                    if (flag)
                    {
                        Main.gunLocked = true;
                ...
                bool flag2 = Main.gunLocked && Main.lockTarget != null;
                if (flag2)
                {
                    NetPlayer creator = Main.lockTarget.Creator;
                    bool flag3 = creator != null;
                    if (flag3)
                    {
                        ...
                        float num = (Main.lockTarget.GetComponent<Rigidbody>() != null)
                            ? Main.lockTarget.GetComponent<Rigidbody>().velocity.magnitude
                            : 0f;
                        float num2 = Vector3.Distance(GorillaTagger.Instance.offlineVRRig.transform.position,
                            Main.lockTarget.transform.position);
                        string text3 = (Main.lockTarget.setMatIndex == 1)
                            ? "LAVA"
                            : ((Main.lockTarget.setMatIndex == 2)
                                ? "ROCK"
                                : ((Main.lockTarget.setMatIndex == 3) ? "WATER" : "NO"));
                        float num3 = 1f / Time.unscaledDeltaTime;
                        int num4 = 0;
                        MeshRenderer[] componentsInChildren =
                            Main.lockTarget.GetComponentsInChildren<MeshRenderer>(true);
                        foreach (MeshRenderer meshRenderer in componentsInChildren)
                        {
                            bool flag4 = meshRenderer != null && meshRenderer.gameObject.activeInHierarchy &&
                                         meshRenderer.transform.parent != null;
                            if (flag4)
                            {
                                bool flag5 = meshRenderer.transform.parent.name.ToLower().Contains("cosmetic");
                                if (flag5)
                                {
                                    num4++;
                                }
                        ...
                        string text4 = string.Concat(new string[]
                        {
                            ...
                            text,
                            "\nCOLOR: ",
                            Mathf.RoundToInt(Main.lockTarget.playerColor.r * 9f).ToString(),
                            ",",
                            Mathf.RoundToInt(Main.lockTarget.playerColor.g * 9f).ToString(),
                            ",",
                            Mathf.RoundToInt(Main.lockTarget.playerColor.b * 9f).ToString(),
                            "\nTAGGED: ",
                            text3,
                            "\nMASTER: ",
                            text2,
                            "\nActor: ",
                            creator.ActorNumber.ToString(),
                            "\nITEMS: ",
                            num4.ToString(),
                            "\nFPS: ",
                            Mathf.Ceil(num3).ToString(),
                            "\nSPEED: ",
                            num.ToString("F1"),
                            "\nDIST: ",
                            num2.ToString("F2")
                        });
                        TextMesh textMesh = Main.lockTarget.headMesh.GetComponentInChildren<TextMesh>();
                        bool flag6 = textMesh == null;
                        if (flag6)
                        {
                            GameObject gameObject = new GameObject("PlayerInfoText");
                            gameObject.transform.SetParent(Main.lockTarget.headMesh.transform);
                        ...
                        textMesh.text = text4;
                        textMesh.transform.rotation = Quaternion.LookRotation(textMesh.transform.position -
                                                                              GorillaTagger.Instance.mainCamera
                                                                                  .transform.position);
                    }
                }
            }
            else
            {
                ...
                bool flag7 = Main.lockTarget != null;
                if (flag7)
                {
        ...
        public static void playerinfohandgun()
        {
            ...
            if (gunInput)
            {
                ...
                if (flag2)
                {
                    ...
                    if (flag3)
                    {
                        ...
                        if (flag6)
                        {
                            GameObject gameObject = new GameObject("PlayerInfoText_BIG");
                            gameObject.transform.SetParent(Main.lockTarget.headMesh.transform);
                            gameObject.transform.localPosition = Vector3.up * 1.2f;
                            textMesh = gameObject.AddComponent<TextMesh>();
            ...
            else
            {
                ...
                if (flag7)
                {
                    Transform transform = Main.lockTarget.headMesh.transform.Find("PlayerInfoText_BIG");
        ...
        public static void playerinfohandgunV2()
        {
            ...
            if (gunInput)
            {
                ...
                if (flag2)
                {
                    ...
                    if (flag3)
                    {
                        ...
                        GameObject gameObject = GameObject.Find("MyRightHandInfoDisplay");
                        bool flag6 = gameObject == null;
                        if (flag6)
                        {
                            gameObject = new GameObject("MyRightHandInfoDisplay");
                            ...
                            textMesh.characterSize = 0.0035f;
                            textMesh.alignment = TextAlignment.Left;
                        ...
                        gameObject.transform.LookAt(GorillaTagger.Instance.mainCamera.transform);
            ...
            else
            {
                Main.gunLocked = false;
                Main.lockTarget = null;
                GameObject gameObject2 = GameObject.Find("MyRightHandInfoDisplay");
                bool flag7 = gameObject2 != null;
                if (flag7)
                {
                    ...
                    if (flag3)
                    {
                        ...
                        string text3 = (Main.lockTarget.setMatIndex == 1)
                            ...
                            : ((Main.lockTarget.setMatIndex == 2)
                                ...
                                : ((Main.lockTarget.setMatIndex == 3) ? "WATER" : "NO"));
                        float num3 = 1f / Time.unscaledDeltaTime;
                        System.Random random = new System.Random(creator.UserId.GetHashCode());
                        int num4 = random.Next(2021, 2024);
                        string text4 = random.Next(1, 13).ToString() + "/??/" + num4.ToString();
                        int num5 = 0;
                        MeshRenderer[] componentsInChildren =
                            Main.lockTarget.GetComponentsInChildren<MeshRenderer>(true);
                        foreach (MeshRenderer meshRenderer in componentsInChildren)
        ...
        public static void playerinfogunnarrarate()
        {
            if (Main.GetGunInput(false))
            {
                ...
                {
                    VRRig componentInParent = item.collider.GetComponentInParent<VRRig>();
                    if (componentInParent && !componentInParent.isOfflineVRRig)
                    {
                        Main.gunLocked = true;
                        Main.lockTarget = componentInParent;
                    }
                ...
                if (Main.gunLocked && Main.lockTarget != null)
                {
                    ...
                    if (creator != null)
                    {
                        string platform = (Main.lockTarget.GetPlatform() == "Standalone") ? "Quest" : "Steam";
                        float speed = (Main.lockTarget.GetComponent<Rigidbody>() != null)
                            ? Main.lockTarget.GetComponent<Rigidbody>().velocity.magnitude
                            : 0f;
                        float dist = Vector3.Distance(GorillaTagger.Instance.offlineVRRig.transform.position,
                            Main.lockTarget.transform.position);
                        string material = (Main.lockTarget.setMatIndex == 1)
                            ? "Lava"
                            : ((Main.lockTarget.setMatIndex == 2)
                                ? "Rock"
                                : ((Main.lockTarget.setMatIndex == 3) ? "Water" : "None"));
                        System.Random random = new System.Random(creator.UserId.GetHashCode());
                        int year = random.Next(2021, 2024);
                        string creationDate = random.Next(1, 13).ToString() + "/??/" + year.ToString();
                        int itemCount = 0;
                        MeshRenderer[] meshes = Main.lockTarget.GetComponentsInChildren<MeshRenderer>(true);
                        foreach (MeshRenderer mr in meshes)
                        {
                            if (mr != null && mr.gameObject.activeInHierarchy && mr.transform.parent != null &&
                                mr.transform.parent.name.ToLower().Contains("cosmetic"))
                                itemCount++;
                        }

                        string narration = "Target: " + creator.NickName + ". Platform: " + platform +
                            ". Distance: " + dist.ToString("F1") + " meters. Speed: " + speed.ToString("F1") +
                            ". Cosmetics: " + itemCount + ". Material: " + material + ".";
                        Main.SpeakText(narration);
                    }
                }
            }
            else
        ...
        public static void playerinfohandgunV4()
        {
            ...
            if (gunInput)
            {
                ...
                if (flag2)
                {
                    ...
                    if (flag3)
                    {
                        ...
                        string text4 = string.Concat(new string[]
                        {
                            "<color=#00ff88>◆ TARGET ACQUIRED ◆</color>\n",
                            "<color=#555555>━━━━━━━━━━━━━━━━━━━━━</color>\n",
                            "  <color=#88ddff>NAME</color>     <color=#ffffff>",
                            creator.NickName,
                            "</color>\n  <color=#88ddff>ID</color>       <color=#ffffff>",
                            creator.UserId,
                            "</color>\n  <color=#88ddff>PLATFORM</color> <color=#ffffff>",
                            text,
                            "</color>\n  <color=#88ddff>COLOR</color>    <color=#ffffff>",
                            Mathf.RoundToInt(Main.lockTarget.playerColor.r * 9f).ToString(),
                            ",",
                            Mathf.RoundToInt(Main.lockTarget.playerColor.g * 9f).ToString(),
                            ",",
                            Mathf.RoundToInt(Main.lockTarget.playerColor.b * 9f).ToString(),
                            "</color>\n  <color=#88ddff>TAGGED</color>   <color=#ffffff>",
                            text3,
                            "</color>\n  <color=#88ddff>MASTER</color>   ",
                            text2,
                            "\n  <color=#88ddff>ACTOR</color>    <color=#ffffff>#",
                            creator.ActorNumber.ToString(),
                            "</color>\n  <color=#88ddff>ITEMS</color>    <color=#ffffff>",
                            num4.ToString(),
                            "</color>\n",
                            "<color=#555555>━━━━━━━━━━━━━━━━━━━━━</color>\n",
                            "  <color=#88ddff>FPS</color>      <color=#ffffff>",
                            Mathf.Ceil(num3).ToString(),
                            " </color> <color=#88ddff>SPD</color> <color=#ffffff>",
                            num.ToString("F1"),
                            "</color>\n  <color=#88ddff>DIST</color>     <color=#ffffff>",
                            num2.ToString("F2"),
                            "m</color>"
                        });
                        Transform rightHandTransform = GorillaTagger.Instance.rightHandTransform;
                        string displayName = "PlayerInfoHUD_Realistic";
                        GameObject gameObject = GameObject.Find(displayName);
                        bool flag6 = gameObject == null;
                        if (flag6)
                        {
                            gameObject = new GameObject(displayName);
                            gameObject.transform.SetParent(rightHandTransform);
                            gameObject.transform.localPosition = new Vector3(-0.03f, -0.08f, 0.06f);
                            gameObject.transform.localRotation = Quaternion.Euler(20f, 0f, -10f);

                            GameObject gameObject2 = GameObject.CreatePrimitive(PrimitiveType.Quad);
                            gameObject2.name = "HUD_Backplate";
                            gameObject2.transform.SetParent(gameObject.transform);
                            gameObject2.transform.localPosition = Vector3.zero;
                            gameObject2.transform.localScale = new Vector3(0.065f, 0.09f, 1f);
                            gameObject2.transform.localRotation = Quaternion.identity;
                            Renderer renderer = gameObject2.GetComponent<Renderer>();
                            renderer.material = new Material(Shader.Find("Unlit/Color"));
                            renderer.material.color = new Color(0.03f, 0.03f, 0.06f, 0.85f);
                            Object.Destroy(gameObject2.GetComponent<Collider>());

                            GameObject gameObject3 = GameObject.CreatePrimitive(PrimitiveType.Quad);
                            gameObject3.name = "HUD_Frame";
                            gameObject3.transform.SetParent(gameObject.transform);
                            gameObject3.transform.localPosition = new Vector3(0f, 0f, -0.001f);
                            gameObject3.transform.localScale = new Vector3(0.068f, 0.093f, 1f);
                            gameObject3.transform.localRotation = Quaternion.identity;
                            Renderer renderer2 = gameObject3.GetComponent<Renderer>();
                            renderer2.material = new Material(Shader.Find("Unlit/Color"));
                            renderer2.material.color = new Color(0f, 0.8f, 0.45f, 0.5f);
                            Object.Destroy(gameObject3.GetComponent<Collider>());

                            GameObject gameObject4 = new GameObject("HUD_Scanline");
                            gameObject4.transform.SetParent(gameObject.transform);
                            gameObject4.transform.localPosition = new Vector3(-0.03f, 0.04f, 0.002f);
                            ...
                            textMesh.font = Resources.GetBuiltinResource<Font>("Arial.ttf");
                            textMesh.GetComponent<MeshRenderer>().material = textMesh.font.material;
                            textMesh.color = new Color(0f, 0.9f, 0.5f, 0.3f);
                            textMesh.text = "▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄";

                            GameObject gameObject5 = new GameObject("HUD_Text");
                            gameObject5.transform.SetParent(gameObject.transform);
                            gameObject5.transform.localPosition = new Vector3(-0.028f, 0.035f, 0.002f);
                            TextMesh textMesh2 = gameObject5.AddComponent<TextMesh>();
                            textMesh2.fontSize = 36;
                            textMesh2.characterSize = 0.001f;
                            textMesh2.alignment = TextAlignment.Left;
                            textMesh2.anchor = TextAnchor.UpperLeft;
                            textMesh2.font = Resources.GetBuiltinResource<Font>("Arial.ttf");
                            textMesh2.GetComponent<MeshRenderer>().material = textMesh2.font.material;
                            textMesh2.color = Color.white;
                        }

                        TextMesh component = gameObject.transform.Find("HUD_Text").GetComponent<TextMesh>();
                        component.text = text4;
                        gameObject.transform.LookAt(GorillaTagger.Instance.mainCamera.transform);
                        gameObject.transform.Rotate(0f, 180f, 0f);
                    }
            ...
            else
            {
                Main.gunLocked = false;
                Main.lockTarget = null;
                GameObject gameObject6 = GameObject.Find("PlayerInfoHUD_Realistic");
                bool flag7 = gameObject6 != null;
                if (flag7)
                {
        ...
        public static void playerinfoline()
        {
            ...
            if (flag2)
            {
                ...
                Vector3 vector = controllerTransform.position + controllerTransform.forward * 100f;
                RaycastHit raycastHit;
                bool flag4 = Physics.Raycast(controllerTransform.position, controllerTransform.forward, out raycastHit);
                if (flag4)
                {
                    ...
                    if (flag5)
                    {
                        ...
                        bool flag6 = componentInParent != null && !componentInParent.isOfflineVRRig;
                        if (flag6)
                        {
                            Main.gunLocked = true;
                            Main.lockTarget = componentInParent;
            ...
            {
                GameObject gameObject2 = GameObject.Find("InfoLaser");
                bool flag7 = gameObject2 != null;
                if (flag7)
                {
                    Object.Destroy(gameObject2);
            ...
            bool flag8 = Main.gunLocked && Main.lockTarget != null;
            if (flag8)
            {
                NetPlayer creator = Main.lockTarget.Creator;
                bool flag9 = creator != null;
                if (flag9)
                {
                    string text = (Main.lockTarget.GetPlatform() == "Standalone") ? "QUEST" : "STEAM";
                    string text2 = creator.IsMasterClient ? "<color=yellow>TRUE</color>" : "FALSE";
                    ...
                    float num3 = 1f / Time.unscaledDeltaTime;
                    int num4 = 0;
                    MeshRenderer[] componentsInChildren = Main.lockTarget.GetComponentsInChildren<MeshRenderer>(true);
                    foreach (MeshRenderer meshRenderer in componentsInChildren)
                    {
                        bool flag10 = meshRenderer != null && meshRenderer.gameObject.activeInHierarchy &&
                                      meshRenderer.transform.parent != null;
                        if (flag10)
                        {
                            bool flag11 = meshRenderer.transform.parent.name.ToLower().Contains("cosmetic");
                            if (flag11)
                            {
                                num4++;
                            }
                    ...
                    GameObject gameObject3 = GameObject.Find("HandScannerDisplay");
                    bool flag12 = gameObject3 == null;
                    if (flag12)
                    {
                        gameObject3 = new GameObject("HandScannerDisplay");
                        gameObject3.transform.SetParent(controllerTransform);
                        gameObject3.transform.localPosition = new Vector3(0f, 0.15f, 0.05f);
                    }

                    TextMesh textMesh = gameObject3.GetComponent<TextMesh>();
                    bool flag13 = textMesh == null;
                    if (flag13)
                    {
                        textMesh = gameObject3.AddComponent<TextMesh>();
                        textMesh.fontSize = 60;
                    ...
                    textMesh.transform.LookAt(GorillaTagger.Instance.mainCamera.transform);
                    textMesh.transform.Rotate(0f, 180f, 0f);
                }


                bool flag14 = rightControllerPrimaryButton;
                if (flag14)
                {
                    Main.gunLocked = false;
                    Main.lockTarget = null;
                GameObject gameObject4 = GameObject.Find("MyRightHandInfoLineDisplay");
                    bool flag15 = gameObject4 != null;
                    if (flag15)
                    {
        ...
        public static void playerinfolineV2()
        {
            ...
            if (flag8)
            {
                ...
                if (flag9)
                {
                    ...
                    string newFullText = string.Concat(new string[]
                    {
                        "<color=red>TARGET LOCKED</color>\nNAME: ",
                        creator.NickName,
                        "\nID: ",
                    ...
                    Transform rightHandTransform = GorillaTagger.Instance.rightHandTransform;
                    GameObject gameObject3 = GameObject.Find("MyRightHandInfoLineDisplay");
                    bool flag12 = gameObject3 == null;
                    if (flag12)
                    {
                        gameObject3 = new GameObject("MyRightHandInfoLineDisplay");
                        gameObject3.transform.SetParent(rightHandTransform);
                        gameObject3.transform.localPosition = new Vector3(0f, 0.15f, 0f);
                    }
                    ...
                    if (creator.UserId != typewriterTargetId)
                    {
                        ...
                        typewriterFullText = newFullText;
                        typewriterIndex = 0;
                        typewriterTimer = 0f;
                    }
                    ...
                    textMesh.text = typewriterFullText.Substring(0, typewriterIndex);
        ...
        {
            Transform handTransform = GorillaLocomotion.GTPlayer.Instance.RightHand.controllerTransform;
            bool isGripPressed = ControllerInputPoller.instance.rightGrab;
            bool isResetPressed = ControllerInputPoller.instance.rightControllerPrimaryButton;

            if (isResetPressed && !wasResetPressed)
            {
                modEnabled = !modEnabled;
                if (!modEnabled) CleanupAllVisualsV3();
            }
            ...
            {
                cachedEspRigs = UnityEngine.Object.FindObjectsOfType<VRRig>();
            ...
            VRRig closestRigInRange = null;

            if (cachedEspRigs != null)
            {
                foreach (VRRig rig in cachedEspRigs)
                {
                    if (rig == null || rig.isOfflineVRRig || rig.isMyPlayer || rig.Creator == null) continue;
                    float dist = Vector3.Distance(GorillaTagger.Instance.offlineVRRig.transform.position,
                        rig.transform.position);

                    UpdateNameTagV3(rig, dist);

                    if (dist < shortestDistance)
                    {
                        shortestDistance = dist;
                        closestRigInRange = rig;
                    }
                }
            }
            ...
            {
                GameObject textObj = GameObject.Find("HandScannerDisplay");
                if (textObj == null)
                {
                    textObj = new GameObject("HandScannerDisplay");
                    textObj.transform.SetParent(handTransform);
                    textObj.transform.localPosition = new Vector3(0f, 0.20f, 0f);
                    textObj.transform.localRotation = Quaternion.identity;

                    cachedDisplay = textObj.AddComponent<TextMesh>();
                    ...
                    cachedDisplay.GetComponent<MeshRenderer>().material = cachedDisplay.font.material;
                    cachedDisplay.anchor = TextAnchor.LowerCenter;
                }
                else
                {
                    cachedDisplay = textObj.GetComponent<TextMesh>();
                }
            }

            if (closestRigInRange != null)
            {
                var targetPlayer = closestRigInRange.Creator;
                if (isGripPressed)
                {
                    string plat = targetPlayer.UserId.StartsWith("76561") ? "STEAM" : "QUEST";
                    string isMaster = targetPlayer.IsMasterClient ? "<color=yellow>TRUE</color>" : "FALSE";
                    float speed = (closestRigInRange.GetComponent<Rigidbody>() != null)
                        ? closestRigInRange.GetComponent<Rigidbody>().velocity.magnitude
                        : 0f;
                    string tagStatus = (closestRigInRange.setMatIndex == 1) ? "LAVA" :
                        (closestRigInRange.setMatIndex == 2) ? "ROCK" : "NO";
                    float fps = 1.0f / Time.unscaledDeltaTime;

                    if (Time.time > lastCosmeticScan + 1.0f)
                    {
                        cachedWearingCount = 0;
                        foreach (MeshRenderer mesh in closestRigInRange.GetComponentsInChildren<MeshRenderer>(true))
                            if (mesh != null && mesh.gameObject.activeInHierarchy &&
                                mesh.transform.parent?.name.ToLower().Contains("cosmetic") == true)
                                cachedWearingCount++;
                        lastCosmeticScan = Time.time;
                    }

                    System.Random rnd = new System.Random(targetPlayer.UserId.GetHashCode());
                    string creationDate = rnd.Next(1, 13) + "/??/" + rnd.Next(2021, 2026);
                    ...
                    cachedDisplay.text = "<color=red>TARGET LOCKED</color>\n" +
                                         ...
                                         "PLATFORM: " + plat + "\n" +
                                         "COLOR: " + Mathf.RoundToInt(closestRigInRange.playerColor.r * 9f) + "," +
                                         Mathf.RoundToInt(closestRigInRange.playerColor.g * 9f) + "," +
                                         Mathf.RoundToInt(closestRigInRange.playerColor.b * 9f) + "\n" +
                                         "TAGGED: " + tagStatus + "\n" +
                                         "MASTER: " + isMaster + "\n" +
                                         "Actor: " + targetPlayer.ActorNumber + "\n" +
                                         "ITEMS: " + cachedWearingCount + "\n" +
                                         "FPS: " + Mathf.Ceil(fps) + "\n" +
                                         "SPEED: " + speed.ToString("F1") + "\n" +
                                         "DIST: " + shortestDistance.ToString("F2");
                }
                else
                {
                    cachedDisplay.text = "<color=red>ALERT:</color>\n" +
                                         "<color=white>SOMEONE IN CIRCLE</color>\n" +
                                         "<color=green>" + targetPlayer.NickName + "</color>\n" +
            ...
            cachedDisplay.transform.LookAt(GorillaTagger.Instance.mainCamera.transform);
            cachedDisplay.transform.Rotate(0, 180, 0);
        }

        private static void UpdateNameTagV3(VRRig rig, float dist)
        {
            string tagName = "NameTag-" + rig.Creator.UserId;
            GameObject tag = GameObject.Find(tagName);

            if (tag == null)
            {
                tag = new GameObject(tagName);
            ...
            tag.GetComponent<TextMesh>().text = $"<color=green>{rig.Creator.NickName}</color>\n[{Mathf.Floor(dist)}m]";
            tag.transform.position = rig.transform.position + Vector3.up * 0.7f;
        ...
        {
            if (auraCircleObj == null)
            {
                auraCircleObj = new GameObject("AuraCircle");
                circleRenderer = auraCircleObj.AddComponent<LineRenderer>();
                circleRenderer.material = new Material(Shader.Find("GUI/Text Shader"));
                circleRenderer.startColor = circleRenderer.endColor = Color.red;
                circleRenderer.startWidth = 0.025f;
                circleRenderer.positionCount = 31;
                circleRenderer.loop = true;
            }

            Vector3 center = GorillaTagger.Instance.offlineVRRig.transform.position + Vector3.down * 0.85f;
            for (int i = 0; i < 31; i++)
            {
        ...
        private static void CleanupAllVisualsV3()
        {
            ...
            {
                if (obj.name.StartsWith("NameTag-") || obj.name == "HandScannerDisplay" || obj.name == "AuraCircle")
        ...
        {
            if (!InfoPlayerModEnabled) return;
            ...
            if (cachedEspRigs != null)
            {
                ...
                {
                    VRRig rig = cachedEspRigs[i];
                    if (rig == null || rig.isOfflineVRRig || rig.isMyPlayer || rig.Creator == null) continue;

                    float dist = Vector3.Distance(GorillaTagger.Instance.offlineVRRig.transform.position,
                        rig.transform.position);

                    UpdateNameTagV4(rig, dist);
            ...
            UpdateTracer(closestRigInRange, handTransform.position);

            if (cachedDisplay == null)
            {
                GameObject textObj = new GameObject("HandScannerDisplay");
                textObj.transform.SetParent(handTransform);
                textObj.transform.localPosition = new Vector3(0f, 0.20f, 0f);
                textObj.transform.localRotation = Quaternion.identity;
            ...
            if (closestRigInRange != null)
            {
                ...
                if (isGripPressed)
                {
                    ...
                    if (Time.time > lastCosmeticScan + 2.0f)
                    {
                        cachedWearingCount = 0;
                        MeshRenderer[] meshes = closestRigInRange.GetComponentsInChildren<MeshRenderer>(true);
                        for (int i = 0; i < meshes.Length; i++)
                        {
                            if (meshes[i] != null && meshes[i].gameObject.activeInHierarchy &&
                                meshes[i].transform.parent?.name.ToLower().Contains("cosmetic") == true)
                                cachedWearingCount++;
                        }
        ...
        private static void UpdateTracer(VRRig target, Vector3 startPos)
        {
            ...
            if (target != null && tracerRenderer != null)
            {
                tracerRenderer.enabled = true;
                tracerRenderer.startColor = tracerRenderer.endColor = Color.red;
                tracerRenderer.SetPosition(0, startPos);
                tracerRenderer.SetPosition(1, target.transform.position);
            }
            else if (tracerRenderer != null)
            {
        ...
        {
            string tagName = "NT_" + rig.Creator.UserId;
            GameObject tag = GameObject.Find(tagName);
        ...
        {
            if (tracerObj != null) UnityEngine.Object.Destroy(tracerObj);
            tracerObj = null;
            tracerRenderer = null;
            cachedDisplay = null;
        ...
        public static void ApplyChams()
        {
            ...
            {
                if (rig.isLocal) continue;
                Renderer[] rends = rig.GetComponentsInChildren<Renderer>();
                foreach (Renderer r in rends)
                {
                    if (originalMaterials.ContainsKey(r)) continue;
                    originalMaterials[r] = new[] { r.material };
                    if (!uberChams) uberChams = LoadAsset<Shader>("UberChams");
                    r.material = new Material(uberChams);
        ...
        public static void CasualBallESP()
        {
            ...
            foreach (var b in ballESP.Where(b => !VRRigCache.ActiveRigs.Contains(b.Key)))
            {
                toRemove.Add(b.Key);
                Object.Destroy(b.Value);
            }
            ...
            foreach (var vrrig in VRRigCache.ActiveRigs.Where(vrrig => !vrrig.isLocal))
            {
                if (!ballESP.TryGetValue(vrrig, out GameObject sphere))
                {
                ...
                sphere.transform.LookAt(GorillaTagger.Instance.headCollider.transform.position);
            }
        }
        ...
        public static void InfectionBallESP()
        {
            ...
            foreach (var vrrig in VRRigCache.ActiveRigs.Where(vrrig => !vrrig.isLocal))
            {
                ...
                bool playerTagged = vrrig.IsTagged();
                sphere.SetActive((selfTagged ? !playerTagged : playerTagged) || InfectedList().Count <= 0);
        ...
        public static void NearestBallESP()
        {
            ...
            bool tt = Buttons.GetIndex("Transparent Theme").enabled;
            List<VRRig> toRemove = new List<VRRig>();
            foreach (var b in ballESP.Where(box => !VRRigCache.ActiveRigs.Contains(box.Key)))
            {
                toRemove.Add(b.Key);
                Object.Destroy(b.Value);
            }

            foreach (VRRig rig in toRemove) ballESP.Remove(rig);
            VRRig target = FindNearest();
            toRemove.Clear();
            foreach (var kvp in ballESP)
            {
                if (kvp.Key != target)
                {
                    toRemove.Add(kvp.Key);
            ...
            foreach (VRRig rig in toRemove) ballESP.Remove(rig);
            if (target == null) return;
            if (!ballESP.TryGetValue(target, out GameObject sphere))
            {
                sphere = GameObject.CreatePrimitive(PrimitiveType.Sphere);
                Object.Destroy(sphere.GetComponent<SphereCollider>());
                sphere.transform.localScale = new Vector3(0.3f, 0.3f, 0.3f);
                sphere.GetComponent<Renderer>().material.shader = Shader.Find("GUI/Text Shader");
                ballESP.Add(target, sphere);
            }

            Color color = target.playerColor;
            if (fmt) color = backgroundColor.GetCurrentColor();
            if (tt) color.a = 0.5f;
            if (hoc) sphere.layer = 19;
            sphere.GetComponent<Renderer>().material.color = color;
        ...
        {
            foreach (KeyValuePair<VRRig, GameObject> kvp in ballESP)
                Object.Destroy(kvp.Value);
            ballESP.Clear();
        }
