...
namespace Seralyth.Managers
{
    public class SoundManager
    {
        ...
        private static AudioSource GetButtonSource()
        {
            ...
            {
                var go = new GameObject("SeralythButtonSound");
                UnityEngine.Object.DontDestroyOnLoad(go);
                _buttonSource = go.AddComponent<AudioSource>();
        ...
        public static void Play(string sound, string outputPath = null, Action<AudioClip> action = null, string buttonText = null, bool overlapHand = false, bool leftOverlap = false, bool global = false)
        {
            ...
            if (doButtonsVibrate)
                GorillaTagger.Instance.StartVibration(rightHand, GorillaTagger.Instance.tagHapticStrength / 2f, 0.05f);
            ...
            try
            {
                ...
                switch (path)
                {
                    ...
                    case string s:
                        ...
                        bool isAbsoluteUrl = s.StartsWith("http");
                        string soundUrl = isAbsoluteUrl ? s : $"{PluginInfo.ServerResourcePath}/{s}";
                        string cachePath = isAbsoluteUrl ? $"Audio/Menu/Buttons/{System.IO.Path.GetFileName(new Uri(s).AbsolutePath)}" : (outputPath ?? s);
                        LoadSoundFromURL(soundUrl, cachePath,
                            action ?? (clip =>
                            {
                                AudioSource btnSrc = GetButtonSource();
                                btnSrc.PlayOneShot(clip, buttonClickVolume / 10f);
                            }
                        ));
                        break;
                }
