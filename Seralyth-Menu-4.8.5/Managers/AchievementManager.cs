                ...
                for (int i = 0; i < achievementCount; i++)
                {
                    ...
                    achievementButtons.Add(
                        new ButtonInfo
                        {
                            ...
                            method = () => {
                                string iconUrl;
                                if (achievement.icon.StartsWith("http") || achievement.icon.StartsWith("file:///"))
                                    iconUrl = achievement.icon;
                                else if (System.IO.File.Exists($"{PluginInfo.BaseDirectory}/{achievement.icon}"))
                                    iconUrl = "file:///" + System.IO.Path.GetFullPath($"{PluginInfo.BaseDirectory}/{achievement.icon}").Replace("\\", "/");
                                else
                                    iconUrl = $"{PluginInfo.ServerResourcePath}/{achievement.icon}";
                                PromptSingle($"{achievement.description}<{iconUrl}>", null, "Done");
                            },
                            isTogglable = false,
